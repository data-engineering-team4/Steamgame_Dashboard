WITH gi AS (
    SELECT game_id, release_date, negative_cnt, positive_cnt FROM raw_data.game_info
), cm AS (
    SELECT game_id, AVG(total_playtime) total_playtime, AVG(last_two_weeks_playtime) last_two_weeks_playtime FROM raw_data.comment_data GROUP BY 1
), gs AS (
    SELECT game_id, AVG(user_cnt) user_cnt, AVG(review_cnt) review_cnt FROM raw_data.game_status GROUP BY 1
)
SELECT
